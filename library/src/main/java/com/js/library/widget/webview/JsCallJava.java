...
public class JsCallJava {
    ...
    private static final String[] IGNORE_UNSAFE_METHODS = {"getClass", "hashCode", "notify",
    ...
    public JsCallJava(Object interfaceObj, String interfaceName) {
        try {
            ...
            sb.append(
                    " init begin\");var a={queue:[],callback:function(){var d=Array.prototype.slice.call(arguments,0);var c=d.shift();var e=d.shift();this.queue[c].apply(this,d);if(!e){delete this.queue[c]}}};");
            ...
            sb.append(mInterfacedName);
            sb.append(
                    " call result, message:miss method name\"}var e=[];for(var h=1;h<f.length;h++){var c=f[h];var j=typeof c;e[e.length]=j;if(j==\"function\"){var d=a.queue.length;a.queue[d]=c;f[h]=d}}var k = new Date().getTime();var l = f.shift();var m=prompt('");
            sb.append(MSG_PROMPT_HEADER);
            sb.append("'+JSON.stringify(");
            sb.append(promptMsgFormat("'" + mInterfacedName + "'", "l", "e", "f"));
            sb.append(
                    "));console.log(\"invoke \"+l+\", time: \"+(new Date().getTime()-k));var g=JSON.parse(m);if(g.CODE!=200){throw\"");
            sb.append(mInterfacedName);
            sb.append(
                    " call result, CODE:\"+g.CODE+\", message:\"+g.result}return g.result};Object.getOwnPropertyNames(a).forEach(function(d){var c=a[d];if(typeof c===\"function\"&&d!==\"callback\"){a[d]=function(){return c.apply(a,[d].concat(Array.prototype.slice.call(arguments,0)))}}});b.");
            sb.append(mInterfacedName);
    ...
    public String call(WebView webView, JSONObject jsonObject) {
        ...
        if (jsonObject != null) {
            try {
                ...
                // 方法匹配失败
                if (currMethod == null) {
                    return getReturn(jsonObject, 500, "not found method(" + sign + ") with valid parameters",
                            time);
                }
                // 数字类型细分匹配
                if (numIndex > 0) {
                ...
                //优先返回详细的错误信息
                if (e.getCause() != null) {
                    return getReturn(jsonObject, 500, "method execute result:" + e.getCause().getMessage(),
                            time);
                }
    ...
    private String getReturn(JSONObject reqJson, int stateCode, Object result, long time) {
        ...
        Log.d(TAG,
                "call time: " + (android.os.SystemClock.uptimeMillis() - time) + ", request: " + reqJson
                        + ", result:" + resStr);
        return resStr;
    }
