...
import com.js.library.common.util.ResourceUtils;
...
public class CountDownProgressView extends TextView {
    ...
    public CountDownProgressView(Context context, AttributeSet attrs) {
        ...
        TypedArray typedArray = getContext()
                .obtainStyledAttributes(attrs, R.styleable.CountDownProgress);
        if (typedArray != null) {
            // 圆实心的颜色属性
            if (typedArray.hasValue(R.styleable.CountDownProgress_circSolidColor)) {
                ...
                //没有这个属性索引 默认是浅灰色
                circSolidColor = typedArray
                        .getColor(R.styleable.CountDownProgress_circSolidColor, ResourceUtils
            ...
            if (typedArray.hasValue(R.styleable.CountDownProgress_circFrameColor)) {
                ...
                //没有这个属性索引 默认是深灰色
                circFrameColor = typedArray
                        .getColor(R.styleable.CountDownProgress_circFrameColor, Color.parseColor("#A9A9A9"));
            }
            ...
            if (typedArray.hasValue(R.styleable.CountDownProgress_textColor)) {
                ...
                textColor = typedArray
                        .getColor(R.styleable.CountDownProgress_textColor, Color.parseColor("#ffffff"));
            }
            ...
            if (typedArray.hasValue(R.styleable.CountDownProgress_progressColor)) {
                ...
                progressColor = typedArray
                        .getColor(R.styleable.CountDownProgress_progressColor, Color.parseColor("#0000FF"));
            }
    ...
    /**
     * 绘制过程 注意：canvas在绘制过程中是一层层覆盖的。所以绘制顺序不要写反了。如果先绘制实心圆在绘制文字在绘制空心圆。这时你会发现文字是看不到了，被覆盖了。
     */
    @Override
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(mArcRectF, -90, 360 * progress / 100, false,
                mPaint); //这里的-90.是方向，大家可以改成0，90，180，-90等就可以看到效果区别
     ...
     * 这里重写onTouchEvent方法来判断点击是否在园内，是方便有需要或刚接触自定义控件的人学习的。 这2种方式都有一个小缺陷就是它计算的其实是矩形范围内。也就是你稍微偏离园外4个角点击都是可以响应的。
     */
    @Override
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (Math.abs(x - (mBounds.centerX())) <= (circRadius) * 2
                        && Math.abs(y - (mBounds.centerY())) <= (circRadius) * 2) {
