...
/**
 * 拼图区域控件 Created by luozhanming on 2018/1/17.
 */
...
public class PictureVerifyView extends AppCompatImageView {
    ...
    private BlurMaskFilter mShadowFilter;
    ...
    public PictureVerifyView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mShadowFilter = new BlurMaskFilter(10, BlurMaskFilter.Blur.NORMAL);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mState == STATE_MOVE || mState == STATE_IDEL || mState == STATE_DOWN
            ...
            mShadowPaint.setMaskFilter(mShadowFilter);
            // 获取位图的Alpha通道图
            Bitmap shadowBitmap = verfityBlock.extractAlpha();
            canvas.drawBitmap(shadowBitmap, blockInfo.left, blockInfo.top, mShadowPaint);
    ...
    private void checkAccess() {
        if (Math.abs(blockInfo.left - shadowInfo.left) < TOLERANCE
                && Math.abs(blockInfo.top - shadowInfo.top) < TOLERANCE) {
            access();
            if (callback != null) {
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN && mMode == Captcha.MODE_NONBAR) {
            if (event.getX() < blockInfo.left || event.getX() > blockInfo.left + blockSize
                    || event.getY() < blockInfo.top || event.getY() > blockInfo.top + blockSize) {
                return false;
            }
        }
