...
class FitsKeyboard implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    public void onGlobalLayout() {
        if (mImmersionBar != null && mImmersionBar.getBarParams() != null && mImmersionBar
                .getBarParams().keyboardEnable) {
            BarConfig barConfig = mImmersionBar.getBarConfig();
            int bottom = 0, keyboardHeight, navigationBarHeight =
                    barConfig.isNavigationAtBottom() ? barConfig.getNavigationBarHeight()
            ...
            if (keyboardHeight != mTempKeyboardHeight) {
                ...
                if (!ImmersionBar
                        .checkFitsSystemWindows(mWindow.getDecorView().findViewById(android.R.id.content))) {
                    if (mChildView != null) {
