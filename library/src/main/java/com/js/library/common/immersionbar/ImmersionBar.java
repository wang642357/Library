package com.js.library.common.immersionbar;
...
import static com.js.library.common.immersionbar.Constants.FLAG_FITS_DEFAULT;
import static com.js.library.common.immersionbar.Constants.FLAG_FITS_STATUS;
import static com.js.library.common.immersionbar.Constants.FLAG_FITS_SYSTEM_WINDOWS;
import static com.js.library.common.immersionbar.Constants.FLAG_FITS_TITLE;
import static com.js.library.common.immersionbar.Constants.FLAG_FITS_TITLE_MARGIN_TOP;
import static com.js.library.common.immersionbar.Constants.IMMERSION_BOUNDARY_COLOR;
import static com.js.library.common.immersionbar.Constants.IMMERSION_ID_NAVIGATION_BAR_VIEW;
import static com.js.library.common.immersionbar.Constants.IMMERSION_ID_STATUS_BAR_VIEW;
import static com.js.library.common.immersionbar.Constants.IMMERSION_MIUI_NAVIGATION_BAR_DARK;
import static com.js.library.common.immersionbar.Constants.IMMERSION_MIUI_STATUS_BAR_DARK;
...
public final class ImmersionBar implements ImmersionCallback {
    ...
    /**
     * 在Activity使用 With immersion bar.
     *
    ...
    /**
     * 在Fragment使用 With immersion bar.
     *
    ...
    /**
     * 在DialogFragment使用 With immersion bar.
     *
    ...
    private void setSpecialBarDarkMode() {
        if (OSUtils.isMIUI6Later()) {
            ...
            if (mBarParams.navigationBarEnable) {
                SpecialBarFontUtils.setMIUIBarDark(mWindow, IMMERSION_MIUI_NAVIGATION_BAR_DARK,
                        mBarParams.navigationBarDarkIcon);
            }
        }
        // 修改Flyme OS状态栏字体颜色
    ...
    private void setupNavBarView() {
        ...
        if (mBarConfig.isNavigationAtBottom()) {
            ...
            params = new FrameLayout.LayoutParams(mBarConfig.getNavigationBarWidth(),
                    FrameLayout.LayoutParams.MATCH_PARENT);
    ...
    /**
     * android 5.0以上解决状态栏和布局重叠问题 Fits windows above lollipop.
     */
    ...
    /**
     * 解决android4.4有导航栏的情况下，activity底部被导航栏遮挡的问题和android 5.0以下解决状态栏和布局重叠问题 Fits windows below
     * lollipop.
     */
    private void fitsWindowsKITKAT() {
        ...
        if (mBarConfig.hasNavigationBar() && mBarParams.navigationBarEnable
    ...
    /**
     * 注册emui3.x导航栏监听函数 Register emui 3 x.
     */
        ...
        if (navigationBarView != null) {
            ...
            if (!show) {
                ...
                if (checkFitsSystemWindows(mDecorView.findViewById(android.R.id.content))) {
                    ...
                    if (!mBarParams.hideNavigationBar) {
                        FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) navigationBarView
                                .getLayoutParams();
                        if (mBarConfig.isNavigationAtBottom()) {
                            params.gravity = Gravity.BOTTOM;
    ...
    /**
     * Sets status bar dark font. 设置状态栏字体颜色，android6.0以上
     */
    ...
    /**
     * 设置导航栏图标亮色与暗色 Sets dark navigation icon.
     */
    ...
    /**
     * 适配状态栏与布局重叠问题 Fits layout overlap.
     */
                ...
                if (view != null) {
                    if (Math.abs(mBarParams.viewAlpha - 0.0f) == 0) {
                        view.setBackgroundColor(
                                ColorUtils.blendARGB(colorBefore, colorAfter, mBarParams.statusBarAlpha));
                    } else {
                        view.setBackgroundColor(
                                ColorUtils.blendARGB(colorBefore, colorAfter, mBarParams.viewAlpha));
                    }
                }
            }
        }
    ...
    /**
     * 取消注册emui3.x导航栏监听函数和软键盘监听 Cancel listener.
     */
    private void cancelListener() {
        if (mActivity != null) {
            ...
            EMUI3NavigationBarObserver.getInstance().removeOnNavigationBarListener(this);
            NavigationBarObserver.getInstance()
                    .removeOnNavigationBarListener(mBarParams.onNavigationBarListener);
        }
    }

    /**
     * 解决底部输入框与软键盘问题 Keyboard enable.
     */
    ...
    /**
     * 判断手机支不支持状态栏字体变色 Is support status bar dark font boolean.
     *
     * @return the boolean
     */
    ...
    /**
     * 判断导航栏图标是否支持变色 Is support navigation icon dark boolean.
     *
     * @return the boolean
     */
    ...
    /**
     * 为标题栏paddingTop和高度增加fixHeight的高度 Sets title bar.
     *
     * @param activity  the activity
    ...
    /**
     * 为标题栏paddingTop和高度增加状态栏的高度 Sets title bar.
     *
     * @param activity the activity
     * @param view     the view
     */
    ...
    /**
     * 为标题栏marginTop增加fixHeight的高度 Sets title bar margin top.
     *
     * @param activity  the activity
     * @param fixHeight the fix height
     * @param view      the view
    ...
    /**
     * 为标题栏marginTop增加状态栏的高度 Sets title bar margin top.
     *
     * @param activity the activity
     * @param view     the view
     */
    ...
    /**
     * 单独在标题栏的位置增加view，高度为fixHeight的高度 Sets status bar view.
     *
     * @param activity  the activity
     * @param fixHeight the fix height
     * @param view      the view
    ...
    /**
     * 单独在标题栏的位置增加view，高度为状态栏的高度 Sets status bar view.
     *
     * @param activity the activity
     * @param view     the view
     */
    ...
    /**
     * 调用系统view的setFitsSystemWindows方法 Sets fits system windows.
     *
     * @param activity        the activity
    ...
    public static void setFitsSystemWindows(Activity activity, boolean applySystemFits) {
        ...
        setFitsSystemWindows(((ViewGroup) activity.findViewById(android.R.id.content)).getChildAt(0),
                applySystemFits);
    }
    ...
    /**
     * 检查布局根节点是否使用了android:fitsSystemWindows="true"属性 Check fits system windows boolean.
     *
     * @param view the view
     * @return the boolean
     */
    ...
    /**
     * Has navigtion bar boolean. 判断是否存在导航栏
     *
     * @param activity the activity
     * @return the boolean
     */
    ...
    /**
     * Gets navigation bar height. 获得导航栏的高度
     *
     * @param activity the activity
    ...
    /**
     * Gets navigation bar width. 获得导航栏的宽度
     *
     * @param activity the activity
    ...
    /**
     * Is navigation at bottom boolean. 判断导航栏是否在底部
     *
     * @param activity the activity
     * @return the boolean
     */
    ...
    /**
     * Gets status bar height. 或得状态栏的高度
     *
     * @param activity the activity
    ...
    /**
     * Gets action bar height. 或得ActionBar得高度
     *
     * @param activity the activity
    ...
    /**
     * 是否是刘海屏 Has notch screen boolean. e.g:getWindow().getDecorView().post(() ->
    ...
    /**
     * 是否是刘海屏 Has notch screen boolean.
     *
     * @param view the view
     * @return the boolean
     */
    ...
    /**
     * 刘海屏高度 Notch height int. e.g: getWindow().getDecorView().post(() ->
    ...
    /**
     * 在Activity里初始化 Instantiates a new Immersion bar.
     *
     * @param activity the activity
     */
    ...
    /**
     * 在Fragment里初始化 Instantiates a new Immersion bar.
     *
     * @param fragment the fragment
     */
    ...
    /**
     * 在dialogFragment里使用 Instantiates a new Immersion bar.
     *
     * @param dialogFragment the dialog fragment
     */
    ...
    /**
     * 在Dialog里初始化 Instantiates a new Immersion bar.
     *
     * @param activity the activity
     * @param dialog   the dialog
     */
    ...
    /**
     * 状态栏颜色 Status bar color int immersion bar.
     *
    ...
    public ImmersionBar barColor(String barColor,
                                 ...
                                 @FloatRange(from = 0f, to = 1f) float barAlpha) {
        return this
                .barColorInt(Color.parseColor(barColor), Color.parseColor(barColorTransform), barAlpha);
    }
    ...
    public ImmersionBar statusBarColorTransform(@ColorRes int statusBarColorTransform) {
        return this
                .statusBarColorTransformInt(ContextCompat.getColor(mActivity, statusBarColorTransform));
    }
    ...
    public ImmersionBar navigationBarColorTransform(@ColorRes int navigationBarColorTransform) {
        return this.navigationBarColorTransformInt(
                ContextCompat.getColor(mActivity, navigationBarColorTransform));
    }
     ...
     */
    public ImmersionBar addViewSupportTransformColor(View view,
                                                     @ColorRes int viewColorBeforeTransform,
                                                     @ColorRes int viewColorAfterTransform) {
        return this.addViewSupportTransformColorInt(view,
     ...
     * @return the immersion bar
     */
    public ImmersionBar addViewSupportTransformColorInt(View view,
     ...
     */
    public ImmersionBar addViewSupportTransformColorInt(View view,
                                                        @ColorInt int viewColorBeforeTransform,
                                                        @ColorInt int viewColorAfterTransform) {
        if (view == null) {
            throw new IllegalArgumentException("View参数不能为空");
        }
    ...
    /**
     * view透明度 View alpha immersion bar.
     *
    ...
    /**
     * 是否启用自动根据StatusBar和NavigationBar颜色调整深色模式与亮色模式 Auto dark mode enable immersion bar.
     *
     ...
     */
    public ImmersionBar autoDarkModeEnable(boolean isEnable,
                                           @FloatRange(from = 0f, to = 1f) float autoDarkModeAlpha) {
    ...
    /**
     * 是否启用自动根据StatusBar颜色调整深色模式与亮色模式 Auto status bar dark mode enable immersion bar.
     *
     * @param isEnable the is enable
     * @return the immersion bar
     */
    ...
    /**
     * 是否启用自动根据StatusBar颜色调整深色模式与亮色模式 Auto navigation bar dark mode enable immersion bar.
     *
     * @param isEnable the is enable
     * @return the immersion bar
     */
    ...
    /**
     * 是否启用自动根据NavigationBar颜色调整深色模式与亮色模式 Auto navigation bar dark mode enable immersion bar.
     *
     * @param isEnable          the is enable
     * @param autoDarkModeAlpha the auto dark mode alpha
     * @return the immersion bar
    ...
    /**
     * 状态栏字体深色或亮色，判断设备支不支持状态栏变色来设置状态栏透明度 Status bar dark font immersion bar.
     *
    ...
    /**
     * 导航栏图标深色或亮色，只支持android o以上版本 Navigation bar dark icon immersion bar.
     *
    ...
    /**
     * 导航栏图标深色或亮色，只支持android o以上版本，判断设备支不支持导航栏图标变色来设置导航栏透明度 Navigation bar dark icon immersion bar.
     *
     * @param isDarkIcon      the is dark icon
    ...
    /**
     * 修改 Flyme OS系统手机状态栏字体颜色，优先级高于statusBarDarkFont(boolean isDarkFont)方法 Flyme os status bar font
     * color immersion bar.
     *
    ...
    public ImmersionBar flymeOSStatusBarFontColor(@ColorRes int flymeOSStatusBarFontColor) {
        mBarParams.flymeOSStatusBarFontColor = ContextCompat
                .getColor(mActivity, flymeOSStatusBarFontColor);
     ...
     * 默认不会使用该方法，如果是渐变色状态栏和顶部图片请不要调用此方法或者设置为false Apply system fits immersion bar.
     *
     * @param applySystemFits the apply system fits
     * @return the immersion bar
     */
    ...
    /**
     * 解决布局与状态栏重叠问题，支持侧滑返回 Fits system windows immersion bar.
     *
     * @param fits         the fits
    ...
    /**
     * 解决布局与状态栏重叠问题，支持侧滑返回 Fits system windows int immersion bar.
     *
     * @param fits         the fits
     * @param contentColor the content color 整体界面背景色
     * @return the immersion bar
    ...
    /**
     * 通过状态栏高度动态设置状态栏布局 Status bar view immersion bar.
     *
     * @param viewId   the view id
    ...
    /**
     * 解决状态栏与布局顶部重叠又多了种方法 Title bar immersion bar.
     *
     * @param view the view
     * @return the immersion bar
     */
    ...
    /**
     * 解决状态栏与布局顶部重叠又多了种方法，只支持Activity Title bar immersion bar.
     *
     * @param viewId the view id
     * @return the immersion bar
     */
    ...
    public ImmersionBar titleBar(@IdRes int viewId, boolean statusBarColorTransformEnable) {
        if (mSupportFragment != null && mSupportFragment.getView() != null) {
            return titleBar(mSupportFragment.getView().findViewById(viewId),
    ...
    /**
     * 解决状态栏与布局顶部重叠又多了种方法，支持任何view Title bar immersion bar.
     *
     * @param viewId                        the view id
     * @param rootView                      the root view
     * @param statusBarColorTransformEnable the status bar flag 默认为true false表示状态栏不支持变色，true表示状态栏支持变色
     ...
     */
    public ImmersionBar titleBar(@IdRes int viewId, View rootView,
                                 boolean statusBarColorTransformEnable) {
        return titleBar(rootView.findViewById(viewId), statusBarColorTransformEnable);
    }

    /**
     * 绘制标题栏距离顶部的高度为状态栏的高度 Title bar margin top immersion bar.
     *
    ...
    /**
     * 支持有actionBar的界面,调用该方法，布局讲从actionBar下面开始绘制 Support action bar immersion bar.
     *
    ...
    /**
     * 给某个页面设置tag来标识这页bar的属性. Add tag bar tag.
     *
     * @param tag the tag
     * @return the bar tag
     */
    ...
    /**
     * 根据tag恢复到某次调用时的参数 Recover immersion bar.
     *
     * @param tag the tag
     * @return the immersion bar
     */
    ...
    /**
     * 解决软键盘与底部输入框冲突问题 ，默认是false Keyboard enable immersion bar.
     *
    ...
    /**
     * 修改键盘模式 Keyboard mode immersion bar.
     *
     * @param keyboardMode the keyboard mode
     * @return the immersion bar
     */
    ...
    /**
     * 软键盘弹出关闭的回调监听 Sets on keyboard listener.
     *
    ...
    /**
     * 导航栏显示隐藏监听器 Sets on navigation bar listener.
     *
    ...
    public ImmersionBar setOnNavigationBarListener(OnNavigationBarListener onNavigationBarListener) {
        if (onNavigationBarListener != null) {
            if (mBarParams.onNavigationBarListener == null) {
                ...
                NavigationBarObserver.getInstance()
                        .addOnNavigationBarListener(mBarParams.onNavigationBarListener);
            }
        } else {
            if (mBarParams.onNavigationBarListener != null) {
                NavigationBarObserver.getInstance()
    ...
    /**
     * Bar监听，第一次调用和横竖屏切换都会触发此方法，比如可以解决横竖屏切换，横屏情况下，刘海屏遮挡布局的问题 Sets on bar listener.
     *
    ...
    /**
     * 是否可以修改导航栏颜色，默认为true 优先级 navigationBarEnable  > navigationBarWithKitkatEnable >
     * navigationBarWithEMUI3Enable Navigation bar enable immersion bar.
     *
    ...
    /**
     * 是否可以修改4.4设备导航栏颜色，默认为true 优先级 navigationBarEnable  > navigationBarWithKitkatEnable >
    ...
    /**
     * 是否能修改华为emui3.1导航栏颜色，默认为true， 优先级 navigationBarEnable  > navigationBarWithKitkatEnable >
     * navigationBarWithEMUI3Enable Navigation bar with emui 3 enable immersion bar.
     *
    ...
    /**
     * 是否可以使用沉浸式，如果已经是true了，在改为false，之前沉浸式效果不会消失，之后设置的沉浸式效果也不会生效 Bar enable immersion bar.
     *
